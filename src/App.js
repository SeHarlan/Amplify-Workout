...
const App = () => {
  ...
  async function addWorkout() {
    try {
      ...
      await API.graphql(graphqlOperation(createWorkout, { input: workout }))
    } catch (err) {
  ...
  return (
    <div style={styles.container}>
      ...
      />
      <button style={styles.button} onClick={addWorkout}>Create Workout</button>
      {
          ...
          <div key={workout.id ? workout.id : index} style={styles.workout}>
            ...
            <div>
              ...
              <input
                ...
                min="0"
                onChange={({ target }) => {
                  setWeight(index, "light", target.value)
                }}
                style={styles.input}
            ...
            <div>
              ...
              <input
                ...
                min="0"
                onChange={({ target }) => {
                  setWeight(index, "medium", target.value)
                }}
                style={styles.input}
            ...
            <div>
              ...
              <input
                ...
                min="0"
                onChange={({ target }) => {
                  setWeight(index, "heavy", target.value)
                }}
                style={styles.input}
                value={workouts[index].heavy || 0}
              />
            </div>
            <button style={styles.button} onClick={() => removeWorkout(index)}>Delete</button>
          </div>
        ))
      }
    </div>
